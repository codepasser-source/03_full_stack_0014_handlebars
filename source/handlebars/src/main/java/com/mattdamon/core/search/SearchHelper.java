...
import com.mattdamon.core.utils.DateUtiler;
...
public class SearchHelper implements SearchSupport {
	...
	public SearchHelper(Client searchClient) {
	...
	public boolean bulkInsertData(String indexName,
			HashMap<String, Object[]> contentMap) throws GeneralException {

		try {
			logger.consoleLog("------>bulkInsertData");
			...
			logger.consoleLog("[" + indexName + "]" + xContentBuilder.string());
	...
	public boolean bulkDeleteData(String indexName,
			HashMap<String, Object[]> contentMap) throws GeneralException {
		try {
			logger.consoleLog("------>bulkDeleteData");
			QueryBuilder queryBuilder = null;
			queryBuilder = createQueryBuilder(contentMap, SearchLogic.must);
			logger.consoleLog("[" + indexName + "]" + queryBuilder.toString());
	...
	public boolean bulkUpdateData(String indexName,
			...
			HashMap<String, Object[]> newContentMap) throws GeneralException {
		try {
			logger.consoleLog("------>bulkUpdateData");
			// 先删除在插入
	...
	public long getCount(String[] indexNames,
			...
			HashMap<String, Object[]> filterContentMap) throws GeneralException {
	...
	public List<Map<String, Object>> simpleSearch(String[] indexNames,
			...
			String sortType) throws GeneralException {
		...
		try {
			...
			searchRequestBuilder = searchRequestBuilder.setQuery(QueryBuilders
					.wrapperQuery(query));
			logger.consoleLog("[" + indexNames + "]" + query);
	...
	public List<Map<String, Object>> simpleSearch(String[] indexNames,
			HashMap<String, Object[]> searchContentMap,
			HashMap<String, Object[]> filterContentMap, int from, int offset,
			String sortField, String sortType) throws GeneralException {
	...
	public List<String> getSuggest(String[] indexNames, String fieldName,
			String value, int count) throws GeneralException {
		// try {
	...
	public Map<String, String> group(String indexName,
			HashMap<String, Object[]> mustSearchContentMap,
			HashMap<String, Object[]> shouldSearchContentMap,
			String[] groupFields) throws GeneralException {
	 ...
	 *********************************************************/
	/* 格式化数据 */
	private String formatInsertData(Object[] values) throws GeneralException {
		if (!checkValue(values)) {
			return "";
		}
		if (DateUtiler.isDate(values[0])) {
			logger.consoleLog("[" + values[0].toString() + "] formatDate");
	...
	/* 简单的值校验 */
	private boolean checkValue(Object[] values) throws GeneralException {
		if (values == null) {
			return false;
	...
	private QueryBuilder createSingleFieldQueryBuilder(String field,
			Object[] values, SearchOption searchOption) throws GeneralException {
		try {
	...
	private SearchRequestBuilder createHighlight(
			...
			HashMap<String, Object[]> searchContentMap) throws GeneralException {
		Iterator<Entry<String, Object[]>> iterator = searchContentMap
				.entrySet().iterator();
		/* 循环每一个需要搜索的字段和值 */
		while (iterator.hasNext()) {
	...
	private List<Map<String, Object>> getSearchResult(
			SearchResponse searchResponse) throws GeneralException {
		try {
			...
			for (SearchHit searchHit : searchResponse.getHits()) {
				...
				while (highlightIterator.hasNext()) {
					...
					if (contents.length == 1) {
						...
						logger.consoleLog("搜索结果中的高亮结果出现多数据contents.length = "
								+ contents.length);
					}
				}
	...
	private SearchResponse searchCountRequest(String[] indexNames,
			Object queryBuilder) throws GeneralException {
		try {
			SearchRequestBuilder searchRequestBuilder = searchClient
